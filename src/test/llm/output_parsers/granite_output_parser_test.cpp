...
#ifdef _WIN32
const std::string graniteTokenizerPath = getWindowsRepoRootPath() + "\\src\\test\\llm_testing\\Qwen\\Qwen3-8B";
#else
const std::string graniteTokenizerPath = "/ovms/src/test/llm_testing/Qwen/Qwen3-8B";
#endif
...
TEST_F(GraniteOutputParserTest, CompletedThinkingHandsToolCallsToExistingQwen3CoderParser) {
    ...
    const ParsedOutput parsed = test::parseWithStreamer(*graniteTokenizer, *parser,
        encode("reasoning</think><tool_call><function=get_weather></function></tool_call>"), true, true);
